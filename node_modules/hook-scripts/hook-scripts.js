...
function HookScripts() {
  ...
  // If we are running within node.exe as a script "require.main.filename" will
  //  be available.  If we are running as a nexe compiled executable, the path
  //  to the running executable is what we will use as a ScriptName.
  let ScriptName = '';
  if (require.main) {
    let ScriptName = require.main.filename;
  }
  else {
    let ScriptName = process.execPath
  }
  ...
  this.RunScripts = function(EventName) {
    ...
    Q.allSettled(ReaddirPromises).then(function(items) {
      ...
      //TODO: Execute priority sorted scripts one at a time in order.
